  ...
  if (!exifData || !exifData.rawMetadata) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-transparent p-8">
        ...
        <p className="text-sm font-medium text-slate-300">No EXIF data available</p>
        <p className="text-xs text-slate-400 mt-1">Upload an image to view technical details</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-transparent">
      <div className="px-6 py-4 border-b border-white/10 flex justify-between items-center bg-transparent sticky top-0 z-10 backdrop-blur-xl">
        <div>
          <h3 className="text-lg font-bold text-white">Metadata Analysis</h3>
          <p className="text-xs text-white/60 mt-0.5">Technical specifications and hidden tags</p>
        </div>
        <button 
